...
// src/models.ts
var TRASH_ROOT = (0, import_obsidian.normalizePath)("Корзина");
var Trash = class {
  constructor(vault) {
    this.vault = vault;
...
var TrashedBase = class {
  ...
  async restore() {
    ...
    if (restoreParentDir !== "." && !await this.vault.adapter.exists(restoreParentDir)) {
...
var TrashExplorerView = class extends import_obsidian2.ItemView {
  ...
  async restoreFile(item) {
    if (await item.restore()) {
      // Принудительно обновляем интерфейс после восстановления
      setTimeout(async () => {
        await this.forceRefreshUI();
      }, 200);
...
var TrashExplorerPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    ...
    // Устанавливаем настройки для использования папки "Корзина"
    this.setupTrashIntegration();
    ...
    this.addRibbonIcon(
      "trash",
      "Открыть проводник корзины",
      () => this.activateView()
    );
    this.addCommand({
      id: "show-trash-explorer",
      name: "Показать проводник корзины",
    ...
    // Отслеживаем изменения в файловой системе для обновления корзины
    this.registerEvent(
      this.app.vault.on("delete", async (file) => {
        // Только если файл не из корзины, обновляем корзину
  ...
  emptyTrash() {
    const title = "Очистить корзину";
    const message = `Вы уверены что хотите очистить корзину? Все файлы в папке "${TRASH_ROOT}" будут безвозвратно удалены!`;
    return new Promise((resolve) => {
      const confirmModal = new ConfirmModal(
        this.app,
        title,
        ...
        async () => {
          ...
          // Принудительно обновляем интерфейс после очистки корзины
          await this.forceRefreshUI();
          
          new import_obsidian3.Notice(`Корзина очищена`);
          confirmModal.close();
          resolve();
        }
      );
  ...
  setupTrashIntegration() {
    // Создаём настройки плагина
    this.addSettingTab(new TrashExplorerSettingTab(this.app, this));

    // Добавляем контекстное меню для ручного перемещения в корзину
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file && !file.path.startsWith(TRASH_ROOT)) {
          menu.addItem((item) => {
            item
    ...
    // Перехватываем стандартное удаление файлов ТОЛЬКО если включена наша опция
    this.overrideTrashBehavior();
  }

  overrideTrashBehavior() {
    const originalTrash = this.app.vault.trash.bind(this.app.vault);
    const plugin = this;
    
    this.app.vault.trash = async function(file, system) {
      console.log("🗑️ Перехват удаления файла:", file.path);
      console.log("📋 Настройки плагина:", plugin.settings);
      
      // Проверяем настройки нашего плагина
      const useCustomTrash = plugin.settings?.useCustomTrash;
      console.log("✅ Использовать папку Корзина:", useCustomTrash);
      
      // Если включена опция "Использовать папку Корзина" и файл не в корзине
      if (useCustomTrash && !file.path.startsWith(TRASH_ROOT)) {
        console.log("📁 Перемещаем в папку Корзина:", file.path);
        new import_obsidian3.Notice(`Перемещаем "${file.name}" в папку Корзина...`);
        
        try {
          await plugin.moveToTrash(file);
          console.log("✅ Успешно перемещено в Корзина");
          return;
        } catch (error) {
          console.error("❌ Ошибка при перемещении в корзину:", error);
          new import_obsidian3.Notice(`Не удалось переместить "${file.name}" в корзину`);
          // Используем стандартное удаление как fallback
          return await originalTrash(file, system);
        }
      } else {
        console.log("📂 Используем стандартное удаление");
        // Используем стандартную функцию
        return await originalTrash(file, system);
      }
    };
    
    // Регистрируем функцию для восстановления оригинального поведения при отключении плагина
    this.register(() => {
      this.app.vault.trash = originalTrash;
    });
    
    console.log("🔧 Перехват функции trash установлен");
  }
  ...
  async moveToTrash(file) {
    try {
      ...
      if (!await this.app.vault.adapter.exists(TRASH_ROOT)) {
        await this.app.vault.adapter.mkdir(TRASH_ROOT);
      }

      // Генерируем уникальное имя для файла в корзине
      const trashPath = import_obsidian3.normalizePath(`${TRASH_ROOT}/${file.name}`);
      let finalTrashPath = trashPath;
      let counter = 1;

      // Если файл с таким именем уже есть в корзине, добавляем суффикс
      while (await this.app.vault.adapter.exists(finalTrashPath)) {
        const extension = file.extension ? `.${file.extension}` : '';
        const baseName = file.basename;
        finalTrashPath = import_obsidian3.normalizePath(`${TRASH_ROOT}/${baseName} (${counter})${extension}`);
        counter++;
      }

      // Перемещаем файл в корзину
      await this.app.vault.adapter.rename(file.path, finalTrashPath);
      
      // Принудительно обновляем интерфейс Obsidian
      await this.forceRefreshUI();
      
      new import_obsidian3.Notice(`Файл "${file.name}" перемещён в корзину`);
  ...
  async forceRefreshUI() {
    try {
      // Основное обновление файлового проводника
      await this.refreshFileExplorer();
      
      // Обновляем layout workspace
      this.app.workspace.updateLayout();
      
      // Принудительное обновление через небольшую задержку
      setTimeout(async () => {
        // Еще одна попытка обновить файловый проводник
        await this.refreshFileExplorer();
      }, 200);
      
      console.log("🔄 Интерфейс Obsidian обновлён (безопасные методы)");
    } catch (error) {
  ...
  async refreshFileExplorer() {
    try {
      // Ищем файловый проводник более агрессивно
      this.app.workspace.iterateAllLeaves(leaf => {
        if (leaf.view.getViewType() === 'file-explorer') {
          console.log("📂 Найден файловый проводник, обновляем...");
          
          // Пытаемся все доступные методы обновления
          const view = leaf.view;
          if (view.requestSort) view.requestSort();
          if (view.requestRefresh) view.requestRefresh();
          if (view.onFileChange) view.onFileChange();
          if (view.refresh) view.refresh();
          if (view.update) view.update();
          if (view.onResize) view.onResize();
          
          // Принудительно пересоздаём дерево файлов
          if (view.tree && view.tree.refresh) {
            view.tree.refresh();
          }
          
          console.log("✅ Файловый проводник обновлён");
        }
      });
    } catch (error) {
      console.error("❌ Ошибка при обновлении файлового проводника:", error);
    }
  }
...
// Настройки плагина
var TrashExplorerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl('h2', { text: 'Настройки корзины' });

    new import_obsidian3.Setting(containerEl)
      .setName('Использовать папку "Корзина"')
      .setDesc('При включении этой опции удалённые файлы будут перемещаться в папку "Корзина" в корне хранилища вместо стандартной папки ".trash".')
      .addToggle(toggle => toggle
        .setValue(this.plugin.settings.useCustomTrash)
        .onChange(async (value) => {
          this.plugin.settings.useCustomTrash = value;
          await this.plugin.saveSettings();
          if (value) {
            new import_obsidian3.Notice('Теперь удалённые файлы будут помещаться в папку "Корзина"');
          } else {
            new import_obsidian3.Notice('Теперь используется стандартное поведение удаления файлов');
          }
        }));

    containerEl.createEl('p', { 
      text: 'Примечание: Папка "Корзина" будет создана в корне вашего хранилища и будет видима в файловом проводнике.',
      cls: 'setting-item-description'
    });

    containerEl.createEl('p', { 
      text: 'Для управления содержимым корзины используйте команду "Показать проводник корзины" или значок корзины на панели.',
      cls: 'setting-item-description'
    });
  }
};
